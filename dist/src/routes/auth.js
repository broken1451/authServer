"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var express_1 = require("express");
var express_validator_1 = require("express-validator");
var bcrypt_1 = __importDefault(require("bcrypt"));
var validarCampos_1 = require("../middlewares/validarCampos");
var usuarioModel_1 = require("../models/usuarioModel");
var jwt_1 = __importDefault(require("../helpers/jwt"));
var validarToken_1 = require("../middlewares/validarToken");
var authRoutes = express_1.Router();
authRoutes.get("/test", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
        return [2 /*return*/, res.status(200).json({
                ok: true,
            })];
    });
}); });
//login
authRoutes.post("/login", [
    express_validator_1.check("email", "El email es obligatorio").isEmail(),
    express_validator_1.check("password", "El password es obligatorio y debe ser al menos 6 caracteres").isLength({ min: 6 }),
    validarCampos_1.validarCampos,
], function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, email, password, userLogin, payload, token, error_1;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 7, , 8]);
                _a = req.body, email = _a.email, password = _a.password;
                return [4 /*yield*/, usuarioModel_1.UsuarioModel.findOne({ email: email }).exec()];
            case 1:
                userLogin = _b.sent();
                if (!userLogin) return [3 /*break*/, 5];
                if (!userLogin.compararClave(password)) return [3 /*break*/, 3];
                payload = {
                    _id: userLogin._id,
                    name: userLogin.name,
                    email: userLogin.email,
                };
                return [4 /*yield*/, jwt_1.default.generateJwtToken(payload)];
            case 2:
                token = _b.sent();
                return [2 /*return*/, res.status(200).json({
                        ok: true,
                        userLogin: userLogin,
                        token: token,
                    })];
            case 3:
                if (password == "" || userLogin.password == "") {
                    return [2 /*return*/, res.status(400).json({
                            ok: false,
                            mensaje: "Campo vacio",
                            errors: { message: "El campo no puede estar vacio" },
                        })];
                }
                else if (password !== userLogin.password) {
                    return [2 /*return*/, res.status(400).json({
                            ok: false,
                            mensaje: "Clave incorrecta",
                            errors: { message: "Clave incorrecta" },
                        })];
                }
                _b.label = 4;
            case 4: return [3 /*break*/, 6];
            case 5: return [2 /*return*/, res.status(400).json({
                    ok: false,
                    mensaje: "Credenciales no son correctas",
                    errors: { message: "Credenciales no son correctas" },
                })];
            case 6: return [3 /*break*/, 8];
            case 7:
                error_1 = _b.sent();
                console.log({ error: error_1 });
                return [2 /*return*/, res.status(500).json({
                        ok: false,
                        msg: "Ha ocurrido un error interno",
                    })];
            case 8: return [2 /*return*/];
        }
    });
}); });
// nuevo user
authRoutes.post("/new", [
    express_validator_1.check("name", "El nombre es obligatorio").not().isEmpty(),
    express_validator_1.check("email", "El email es obligatorio").isEmail(),
    express_validator_1.check("password", "El password es obligatorio y debe ser al menos 6 caracteres").isLength({ min: 6 }),
    validarCampos_1.validarCampos,
], function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, name_1, email, password, user, salt, usuario, userCreated, token, error_2;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 4, , 5]);
                _a = req.body, name_1 = _a.name, email = _a.email, password = _a.password;
                return [4 /*yield*/, usuarioModel_1.UsuarioModel.findOne({ email: email }).exec()];
            case 1:
                user = _b.sent();
                if (user) {
                    return [2 /*return*/, res.status(400).json({
                            ok: false,
                            msg: "El usuario ya existe con ese correo",
                        })];
                }
                salt = bcrypt_1.default.genSaltSync();
                usuario = {
                    name: name_1,
                    email: email,
                    password: bcrypt_1.default.hashSync(password, salt),
                };
                userCreated = new usuarioModel_1.UsuarioModel(usuario);
                return [4 /*yield*/, jwt_1.default.generateJwtToken(userCreated)];
            case 2:
                token = _b.sent();
                return [4 /*yield*/, userCreated.save()];
            case 3:
                _b.sent();
                return [2 /*return*/, res.status(201).json({
                        ok: true,
                        message: "user guardado",
                        uid: userCreated.id,
                        userCreated: userCreated,
                        token: token,
                    })];
            case 4:
                error_2 = _b.sent();
                console.log({ error: error_2 });
                return [2 /*return*/, res.status(500).json({
                        ok: false,
                        msg: "Ha ocurrido un error interno.",
                    })];
            case 5: return [2 /*return*/];
        }
    });
}); });
// validar y renovar token
authRoutes.get("/renewtoken", validarToken_1.verificaToken, function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var usuario, token, error_3;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                usuario = req.usuario;
                return [4 /*yield*/, jwt_1.default.generateJwtToken(usuario)];
            case 1:
                token = _a.sent();
                delete usuario.password;
                return [2 /*return*/, res.status(200).json({
                        ok: true,
                        msg: "RE new token",
                        usuario: usuario,
                        uid: usuario._id,
                        token: token,
                    })];
            case 2:
                error_3 = _a.sent();
                return [2 /*return*/, res.status(500).json({
                        ok: false,
                        msg: "Ha ocurrido un error interno.",
                    })];
            case 3: return [2 /*return*/];
        }
    });
}); });
exports.default = authRoutes;
